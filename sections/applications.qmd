...
$$ {#eq-3-2-5} 
We can safely ignore any multiplicative constant in $L$ without influencing our inferences regarding $\mu$ and $\rho$. In particular, we can discard the term $\Big(\frac{1}{y_i!}\Big)^{1 - {\mathbbm{1}_{y_i = 0}}}$ as it depends only on the observation $y_i$ and is constant with respect to the model parameters. Hence, we may write 
$$
